...
pub async fn sign_in(
    ...
    // verifica se o retorno da função retrieve_user_by_email tem o mesmo e-mail enviado na requisição
    let user = match retrieve_user_by_email(&user_data.email) {
        Some(user) => user,
        None => return Err(StatusCode::UNAUTHORIZED), // User not found
    };

    //caso sim, verifica se a senha enviada na requisição é igual a senha do usuário mockado


    // 2. Compare the password via bcrypt
